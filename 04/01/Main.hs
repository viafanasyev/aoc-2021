...
playGame :: [Board] -> [Int] -> (Int, Board)
playGame _ [] = undefined
playGame boards (x:xs) =
  case maybeBingoBoard of
    Nothing         -> playGame newBoards xs
    Just bingoBoard -> (x, bingoBoard)
  where
    newBoards = map (mark x) boards
    maybeBingoBoard = find bingo newBoards
...
main = do
  ...
  case args of
    (filePath:_) -> do
      ...
      let (winningVal, winningBoard) = playGame boards values
