...
calc :: Packet -> Int
calc (LiteralPacket _ x)             = x
calc (OperatorPacket _ 0 subPackets) = sum $ map calc subPackets
calc (OperatorPacket _ 1 subPackets) = foldl1 (*) $ map calc subPackets
calc (OperatorPacket _ 2 subPackets) = minimum $ map calc subPackets
calc (OperatorPacket _ 3 subPackets) = maximum $ map calc subPackets
calc (OperatorPacket _ 5 [lhs, rhs]) = if calc lhs >  calc rhs then 1 else 0
calc (OperatorPacket _ 6 [lhs, rhs]) = if calc lhs <  calc rhs then 1 else 0
calc (OperatorPacket _ 7 [lhs, rhs]) = if calc lhs == calc rhs then 1 else 0
