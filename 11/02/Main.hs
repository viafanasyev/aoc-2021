...
step = Map.map (\(fl, v) -> if fl then 0 else v) . flashAll . Map.map (+1)

allFlashedStepNumber :: Map (Int, Int) Int -> Int
allFlashedStepNumber table = if all (==0) $ Map.elems newTable then 1 else 1 + (allFlashedStepNumber newTable)
  where
    newTable = step table
